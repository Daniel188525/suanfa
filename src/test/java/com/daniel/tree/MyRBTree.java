...
public class MyRBTree<T extends Comparable<T>, D> {
    ...
    /**
     * 寻找当前结点的后继节点，即大于该节点的最小节点
     *
     * @param node
     * @return
     */
    public RBNode<T, D> minNodeGreaterThanCurrentNode(RBNode<T, D> node) {
    ...
    public RBNode successor(RBNode<T, D> node) {
        if (node.rightChild != null) {
            return minNodeGreaterThanCurrentNode(node.rightChild);
        }
    ...
    public class RBNode<T extends Comparable<T>, D> {
        private Boolean color; // 节点颜色 true-黑色, false-红色
        private T key; // 键值
        private D data; // 具体的数据
        private RBNode<T, D> parent; // 父亲结点
        private RBNode leftChild; // 左孩子结点
        private RBNode rightChild; // 右孩子结点
